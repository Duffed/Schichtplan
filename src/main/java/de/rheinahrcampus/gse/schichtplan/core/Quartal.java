...
public class Quartal {
	...
	static {
		...
		START_DATUM.put(Quartalszahl.ZWEI, Month.MAY);
		START_DATUM.put(Quartalszahl.DREI, Month.SEPTEMBER);
		
		END_DATUM = new HashMap<>();
		END_DATUM.put(Quartalszahl.EINS, Month.MAY);
		END_DATUM.put(Quartalszahl.ZWEI, Month.SEPTEMBER);
		END_DATUM.put(Quartalszahl.DREI, Month.JANUARY);
	}
	...
	public Quartal(int jahr, Quartalszahl quartalszahl) {
		...
		//Wir bauen ein Datum für den Anfang und das Ende des Quartals
		LocalDate startDatum = LocalDate.of(jahr, startMonat, 1);
		LocalDate endDatum = LocalDate.of(jahr, endMonat, 1);
		
		//In einer SChleife füge ich jeden Tag im Quartal zur Liste tageImQuartal hinzu
	...
	//Liste tageImQuartal kann als Iterable (nicht veränderbar) ausgegeben werden
